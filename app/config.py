...
class Settings:
    ...
    # 重排 provider：local（本地 bge-reranker，需下载模型占用磁盘与 CPU）| siliconflow（在线 API，零下载）
    rerank_provider: str = _env("AIROBOT_RERANK_PROVIDER", "local").lower()
    # local 用 BAAI/bge-reranker-base；siliconflow 用 BAAI/bge-reranker-v2-m3
    rerank_model: str = _env("AIROBOT_RERANK_MODEL", "BAAI/bge-reranker-base")
    rerank_base_url: str = _env("AIROBOT_RERANK_BASE_URL", "https://api.siliconflow.cn/v1")
    rerank_api_key: str = _env("AIROBOT_RERANK_API_KEY")
